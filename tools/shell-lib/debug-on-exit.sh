...
function cluster_info_dump() {
  ...
  KUBECONFIG=$MGMT_KUBECONFIG helm get manifest -n $capi_cluster_namespace sylva-units | yq 'select(.kind!="Secret")' > $dump_dir/helm-release-manifest--sylva-units.yaml

  for helmrelease in cluster cluster-bmh; do
    KUBECONFIG=$MGMT_KUBECONFIG helm get manifest -n $capi_cluster_namespace $helmrelease | yq 'select(.kind!="Secret")' > $dump_dir/helm-release-manifest--$helmrelease.yaml
    KUBECONFIG=$MGMT_KUBECONFIG helm get values -n $capi_cluster_namespace $helmrelease \
      | sed -e 's/ password: .*/ password: redacted # protect the innocent/' \
      > $dump_dir/helm-release-values--$helmrelease.yaml
  done
