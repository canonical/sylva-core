...
SYLVA_TOOLBOX_VERSION=${SYLVA_TOOLBOX_VERSION:-"v0.6.3"}
...
function fetch_ingress_service_types() {
    ...
    for i in $(kubectl --kubeconfig management-cluster-kubeconfig get ingress -A | sed "s/[ \t]\+/|/gi" | grep -v NAMESPACE); do
        ...
        -o json | jq  --arg ingress_name "$ingress_name"  '.metadata.labels | keys[] | select(startswith("sylva-gui-list-service-\($ingress_name)") or endswith("sylva-gui-list-services"))' 2>/dev/null)
