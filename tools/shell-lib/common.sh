...
SYLVA_TOOLBOX_VERSION=${SYLVA_TOOLBOX_VERSION:-"v0.5.13"}
...
function retrieve_kubeconfig {
    ...
    # When management cluster is deployed in libvirt-metal pods hosted in bootstrap cluster,
    # we must use a specific endpoint defined in cluster-public-endpoint configMap to reach the cluster
    CLUSTER_PUBLIC_ENDPOINT=$(kubectl get configmap cluster-public-endpoint --template='{{ .data.endpoint }}' 2> /dev/null)
    if [[ -n $CLUSTER_PUBLIC_ENDPOINT ]]; then
...
function check_management_kubeconfig() {
    if ! kubectl get -n sylva-system kustomization management-flag &>/dev/null; then
...
function fix_sylva_units_deprecated_configuration {
  # This function is necessary only to transition from past Sylva release

  # 1 - sylva-units Helm release value was using "source_templates.sylva-core.existing_source"
  # this setting does not exist any longer in newer versions, where the sylva-core
  ...
  # check if GitRepository exists and needs to be patched
  if [[ $(kubectl -n $ns get gitrepository.source.toolkit.fluxcd.io sylva-core -o 'jsonpath={.metadata.name},{.metadata.labels.app\.kubernetes\.io/managed-by}' 2>/dev/null || true) == "sylva-core," ]]; then
    echo "Patching sylva-core GitRepository to make it owned by sylva-units Helm release..."

    kubectl -n $ns patch gitrepository.source.toolkit.fluxcd.io sylva-core --type=merge \
        '--patch={"metadata":{"labels":{"app.kubernetes.io/managed-by":"Helm"},"annotations":{"meta.helm.sh/release-name":"sylva-units","meta.helm.sh/release-namespace":"'$ns'"}}}'
  fi
  ...
  if [[ -n $INDEX ]]; then
    ...
    kubectl -n $ns patch helmreleases.helm.toolkit.fluxcd.io sylva-units --type=json \
        --patch '[{"op": "remove", "path": "/spec/valuesFrom/'$INDEX'"}]'
  fi
