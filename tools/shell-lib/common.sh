...
function define_source() {
  ...
  if (command -v git && pushd $BASE_DIR && git rev-parse --is-inside-work-tree;) &> /dev/null; then
    ...
    export SYLVA_CORE_TAG=${SYLVA_CORE_TAG:-$(git name-rev --name-only $SYLVA_CORE_COMMIT --refs "refs/tags/*" --no-undefined 2>/dev/null)}
