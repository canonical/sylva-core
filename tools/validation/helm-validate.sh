...
HELM_TOOLBOX_IMAGE=${HELM_TOOLBOX_IMAGE:-registry.gitlab.com/sylva-projects/sylva-elements/container-images/helm-toolbox:1.0.3}

BASE_DIR=$(realpath $(dirname $0)/../../)
#CHART_DIR=$(realpath $(dirname $0)/../../charts/sylva-units --relative-to=$BASE_DIR)
#CHART_DIR=$(realpath "${1:-$(dirname "$0")/../../charts/sylva-units}" --relative-to="$BASE_DIR")
CHART_DIR=$("$BASE_DIR/charts/sylva-units" --relative-to="$BASE_DIR")

# Check if the required arguments are passed
if [ -z "$1" ] || [ -z "$2" ]; then
  echo "Usage: $0 <relative chart-directory> ( schema-validation | yamllint )"
  exit 1
fi

if [[ "$1" = /* ]]; then
  # absolute path
  CHART_DIR=$(realpath "$1"/../../charts/sylva-units --relative-to=$BASE_DIR)
else
  # relative path
  CHART_DIR=$(realpath $(dirname $0)/../../charts/sylva-units --relative-to=$BASE_DIR)
fi
