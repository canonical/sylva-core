...
| **sylva-units-operator** | installs sylva-units operator | experimental |  | [Kustomize](https://gitlab.com/sylva-projects/sylva-elements/sylva-units-operator.git/config/default?ref=0.1.4) | 0.1.4 |
...
| **root-dependency-helm** | special unit ensuring ordered updates of HelmReleases<br/><br/>All HelmReleases depend on the HelmRelease produced by this unit, whose name includes the Helm release version increment.  This ensures that when an upgrade of sylva-units Helm release starts for revision 'n', and the all unit HelmReleases will stop reconciling until their parent Kustomization has reconciled following the reconciliation of its dependencies. |  | True | Helm | N/A |
