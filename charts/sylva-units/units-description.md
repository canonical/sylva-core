...
| **minio-logging-patch-healthCheckExprs** | Patches minio-logging Kustomization to use healthCheckExprs field<br/><br/>This unit was introduced as a workaround for the transition between Flux 2.4 and 2.5.<br/>It patches the minio-logging Kustomization to use the new healthCheckExprs field<br/>that is available in Flux 2.5.x, while removing the older healthChecks field. | Kustomization |
...
| **minio-monitoring-patch-healthCheckExprs** | Patches minio-monitoring Kustomization to use healthCheckExprs field<br/><br/>This unit was introduced as a workaround for the transition between Flux 2.4 and 2.5.<br/>It patches the minio-monitoring Kustomization to use the new healthCheckExprs field<br/>that is available in Flux 2.5.x, while removing the older healthChecks field. | Kustomization |
