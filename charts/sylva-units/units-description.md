...
| :----- | :----- | :----- | :----- |
| **cluster** | holds the Cluster API definition for the cluster | [Helm chart](https://gitlab.com/sylva-projects/sylva-elements/helm-charts/sylva-capi-cluster.git) | 0.4.10 |
| **cluster-bmh** | definitions for Cluster API BareMetalHosts resources (capm3) | [Helm chart](https://gitlab.com/sylva-projects/sylva-elements/helm-charts/sylva-capi-cluster.git) | 0.4.10 |
...
| **cluster-prevent-rke2-helmcharts-calico-metallb** | Kyverno policy to prevent RKE2 HelmCharts from being recreated for MetaLB and Calico<br/><br/>This unit really is a safe-guard. It's not quite sure if it still is needed or not given Sylva design today.<br/>Indeed, we now have only the first node installation install an RKE2 HelmCharts for Calico and MetalLB<br/>and after this Flux takes over. Unless the CAPI RKE2 control plane provider would mistakenly start<br/>the creation of two control plane nodes in parallel, nothing would recreate a HelmChart for those components,<br/>which is highly hypothetical.  There's certainly no reason to keep this policy after the cluster<br/>successfully installed. | Kustomization |
