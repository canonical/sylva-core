...
| **openshift-user-workload-monitoring** | Enables monitoring of CNF components on workload cluster on Sylva management cluster <br/><br/>Generates a secret for thanos receive authentication for management cluster, configmap to configure Prometheus remoteWrite endpoint to management cluster thanos-receive API. Adds the cluster name as a tag to the metrics sent.<br/>Label the namespace with *openshift.io/user-monitoring=true* for enabling monitoring of the components in that namespace. | experimental |  | Kustomize | N/A |
