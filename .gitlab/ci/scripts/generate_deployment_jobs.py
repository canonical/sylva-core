...
def get_mr_comments(project_id, mr_id, HEADERS):
...
def get_ci_configuration_from_context():
    ...
    if "renovate" in os.getenv("CI_MERGE_REQUEST_LABELS", "").split(","):
        ...
        comments = get_mr_comments(GITLAB_PROJECT_ID, GITLAB_MR_ID, HEADERS)
        ...
        if len(user_comments) == 0:
            ...
            for user_comment in user_comments:
                current_user_comment_id = user_comment["id"]
                logging.info(f"Comment(s) found from {current_user_name}, checking if it contain a valid CI configuration")
                ci_config, mr_options = get_ci_config_from_mr_description(user_comment["body"])
                if ci_config:
                    logging.info(f"Using CI configuration from {current_user_name} comment {current_user_comment_id}")
                    return ci_config, mr_options
                else:
