...
def get_client_and_mappers_uuids():
    ...
    for client in all_clients:
        if client.get('secret'):
            client_uuids.update({f"CLIENT_{client['clientId'].replace('-', '_').upper()}": client['id']})
            for mapper in client['protocolMappers']:
                protocol_mappers_uuids.update({f"MAPPER_{mapper['name'].replace(' ','_').upper()}_{client['clientId'].replace('-', '_').upper()}": mapper['id']})
