## OpenShift/OKD Integration Status

The OpenShift CAPI provider supports the installation of both OpenShift and OKD in the same manner. Therefore, in this document, 'OpenShift' and 'OKD' may be used interchangeably, except in the case of the container image pull secret, where OpenShift requires a valid pull secret, while OKD only needs a dummy pull secret. For simplicity, 'OKD' will be used throughout the rest of this document unless 'OpenShift' is explicitly mentioned.
...
Currently, the OpenShift CAPI provider supports only Metal3 as the infrastructure provider. Support for additional infrastructure providers will be added in the future.

## OKD Installer Components

As far as Sylva stack is concerned, there are two components that work together to install OKD. They are OpenShift Assisted Installer and OpenShift Agent CAPI provider.

The OpenShift Agent CAPI provider works like an adaptation layer: it provides the CAPI contract; under the hood it interacts with assisted installer to do the install job. Here is [documentation of the OpenShift CAPI provider architecture]( https://github.com/openshift-assisted/cluster-api-agent/blob/master/docs/architecture_design.md)
...
Since the OpenShift Agent CAPI provider relies on the OpenShift Assisted Installer, it is abbreviated as 'cabpoa' in Sylva.

Currently, 'cabpoa' works only with CAPM3 as the infrastructure provider. Support for additional infrastructure providers will be added in the future.
...
In this section, we will demonstrate an RKE2 management cluster with `cabpoa` and `openshift-assisted-installer` enabled, alongside the OKD workload cluster.
...
```
local=/lab.home/
domain=lab.home                          # {{ .Values.cluster.openshift.baseDomain }} is lab.home
...
Note in the above configuration sample, cluster.name and cluster.openshift.baseDomain will form the DNS entries required for the OKD cluster as mentioned in the DNS settings section.
...
OKD has a default user "core", the purpose of openshift.sshAuthorizedKey is to login to the OKD node as this user.
